...
def add_guest(request):
	...
	if not result:
		return JsonResponse({'ststus':10023,'message':'event status is not avaliable'})
    #发布会限制人数
	event_limit = Event.objects.get(id=eid).limit
	#发布会已添加的嘉宾数
	guest_lime = Guest.objects.filter(event_id=eid)	

	if len(guest_lime) >= event_limit:
	...
	try:
		...
		return JsonResponse({'ststus':10026,'message':'the evnet guest phone number repeat'})
	...
	if eid != ''and phone == '':
		...
		if eid != '' and phone !='':
			...
			try:
				...
				return JsonResponse({'ststus':200,'message':'success','data':guest})
